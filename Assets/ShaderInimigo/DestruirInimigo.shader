Shader "Unlit/DestruirInimigo"
{
    Properties
    {
        _TopTex ("Top", 2D) = "white" {}
        _BackTex ("Boneco", 2D) = "white" {}
        _NoiseTex ("Noise", 2D) = "white" {}
        _DestroyRange ("Destroy Range", Range(-10,10)) = 1
    }
    SubShader
    {
        Pass
        {
           ...
           struct appdata
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float4 col = step(_DestroyRange * noiseTex.x, i.normalWorld.y) * ervaTex + step(i.normalWorld.y, _DestroyRange * noiseTex.x) * pedraTex;
